...
def LogonSpecificIntent(intent, session):
    ...
    if 'accessToken' not in user:
        ...
        if accessToken is None:
            ...
            print "inside else"
            cur.execute("SELECT count(*) from chat_user where auth_code=%s"%accessToken)
            data = cur.fetchone()
            print data[0]
            if data[0] == 0:
                ...
                if card_title == 'Balance':
                    cur.execute("select ab.available_bal from chat_user u,account_balance ab where ab.olb_id=u.olb_id and ab.acct_type='Checking' and u.auth_code=%s"%accessToken)
                    ...
                    if 'shop' in intent['slots']:
                        ...
                        for retail1 in retailname:
                            ...
                            print querystring
                            cur.execute("select sum(td.tran_amount::integer) from transaction_details td,chat_user u where td.olb_id=u.olb_id and u.auth_code= %s and LOWER(td.tran_desc) like %s"%(accessToken,querystring))
                            ...
                            if sum_amount is None:
                                ...
                                date_format = "%Y-%m-%d"
                                cur.execute('select max(td.date) from transaction_details td,chat_user u where u.olb_id=td.olb_id and u.auth_code = %s and LOWER(td.tran_desc) like %s'%(accessToken,querystring))
                                max_date=cur.fetchone()[0]
                                print max_date
                                cur.execute('select min(td.date) from transaction_details td,chat_user u where u.olb_id=td.olb_id and u.auth_code = %s and LOWER(td.tran_desc) like %s'%(accessToken,querystring))
                    ...
                    if 'Number' in intent['slots']:                        
                        if 'value' in intent['slots']['Number']:
                            ...
                            print number
                            cur.execute('select olb_id from chat_user where auth_code=%s'%accessToken)
                    ...
                    if 'Day' in intent['slots']:
                        ...
                        if cur.fetchall():                                                                                                                                                                        
                                ...
                                for row in data:                                                                                        
                                        ...
                                        print outstring
                                        string_result += outstring +','
                                cur.execute('select sum(tran_amount::integer) from payment_details pd, payee_details p where p.payee_id=pd.payee_id and pd.olb_id=%s and pd.payment_date between %s and %s'%("'"+olb_id+"'","'"+startDate+"'","'"+Date+"'"))
                ...
                if('message' not in postbak_msg):
                        if('postback' not in postbak_msg):
                                ...
                                if(link_status=='linked'):
                                        ...
                                        cur.execute('select u.username from chat_user u,fb_chatbot fb where u.auth_code=fb.auth_code and fb.sender_id=%s'%sender_id)
                                ...
                                for w in words:
                                        if(ps.stem(w).lower()=='enrol'):
                                                if 'online' in str(words).lower() and 'banking' in str(words).lower():                                                
                                                        out_msg = 'Enroll to online banking at https://www.newgenbank.com/enroll/olb'
					if(ps.stem(w).lower()=='cashback'):                                                
                                        	out_msg = 'Earn 5% cash back at different places each quarter like gas stations, wholesale clubs, grocery stores, restaurants or Amazon.com, up to the quarterly maximum each time you activate.  Plus earn 1% cash back on all other purchases.'
					if(ps.stem(w).lower()=='credit'):
						if 'low' in str(words).lower():
							out_msg='No issues. Secured card is also an option for you.'
                                                elif 'score' in str(words).lower():                                                
                                                        out_msg = 'Yes its free. Terms and conditions apply.'						
                                        ...
                                        if(ps.stem(w).lower()=='log'):
                                                if 'me' in str(words).lower() and 'out' in str(words).lower():
                                                        ...
                                                        if cur.fetchone():
                                                                flag='Y'
                                                                json_data_final={"recipient":{"id":sender_id},"message": {"attachment": {"type": "template","payload": {"template_type": "generic","elements": [{"title": "Logout","image_url": "https://chatbot-final.herokuapp.com/resources/images/bank-logo.png","buttons": [{"type": "account_unlink"}]}]}}}}                                                                                                                                                                                                                
                                                        else:
                                        ...
                                        if(ps.stem(w).lower()=='pay'):
                                                ...
                                                if cur.fetchone():
                                                        ...
                                                        for subtree in entities.subtrees():                                                                
                                                                ...
                                                                if person_name:
                                                                        if amount:
                                                                                cur.execute('select u.olb_id from chat_user u,fb_chatbot fb where u.auth_code=fb.auth_code and fb.sender_id=%s'%sender_id)
                                                                                olb_id=cur.fetchone()[0]
                                                ...
                                                if cur.fetchone():
                                                        ...
                                                        if cur.fetchall():                                                                                                                                                                        
                                                                ...
                                                                for row in data:                                                                                        
                                                                        ...
                                                                        print outstring
                                                                        string_result += outstring + '\n'
                                                                cur.execute('select sum(tran_amount::NUMERIC) from payment_details pd, payee_details p where p.payee_id=pd.payee_id and pd.olb_id=%s and pd.payment_date between %s and %s'%(olb_id,"'"+startDate+"'","'"+Date+"'"))
                                                                total = cur.fetchone()[0]
                                                                if total is None:
                                                                        total=0
                                                                string_result +='Total due Bill amount is $%s'%total
                                        ...
                                        if(ps.stem(w).lower()=='balanc'):
                                                cur.execute('select u.username from chat_user u,fb_chatbot fb where u.auth_code=fb.auth_code and fb.sender_id=%s'%sender_id)
                                                if cur.fetchone():                                                        
                                                        cur.execute("select ab.available_bal from chat_user u,fb_chatbot fb,account_balance ab where ab.olb_id=u.olb_id and u.auth_code=fb.auth_code and ab.acct_type='Checking' and fb.sender_id=%s"%sender_id)
                                                        balance=cur.fetchone()[0]
                                                ...
                                                if cur.fetchone():
                                                        ...
                                                        if not descstring:
                                                                ...
                                                                for desc in descstring: 
                                                                        ...
                                                                        cur.execute("select sum(tran_amount::numeric) from transaction_details where olb_id = %s and LOWER(tran_desc) like %s"%("'"+olb_id+"'",querystring))
                                                                        balance=cur.fetchone()[0]
                                                                        print balance
                                                                        if balance is None:
                                                                                balance = 0
                                                                                ...
                                                                                date_format = "%Y-%m-%d"
                                                                                cur.execute('select max(date) from transaction_details where olb_id = %s and LOWER(tran_desc) like %s'%("'"+olb_id+"'",querystring))
                                                                                max_date=cur.fetchone()[0]
                                                                                cur.execute('select min(date) from transaction_details where olb_id = %s and LOWER(tran_desc) like %s'%("'"+olb_id+"'",querystring))
                                                                                min_date=cur.fetchone()[0]
                                                                                print min_date
                                                                                a = datetime.datetime.strptime(min_date, date_format).date()
                                                                                print a
...
@app.route('/comericabot', methods=['GET'])
def comericabotverify():
        token = request.args.get('hub.verify_token')
        if token == "123":
                return request.args.get('hub.challenge')
        else:
...
@app.route('/comericabot', methods=["POST"])
def comericawebhookfb():
        try:
                out_msg = 'How may i help you?'
##                natural_language_classifier = NaturalLanguageClassifierV1(
##                username='7a44b06d-6db7-43c3-bc7a-b50586281336',
##                password='vrN6alai6kaI')
                ...
                sender_id= dataDict["entry"][0]["messaging"][0]["sender"]["id"]
                json_data_typing_on={"recipient": {"id": sender_id}, "sender_action": "typing_on"}
                postingMessageComerica(json_data_typing_on);                
                urlparse.uses_netloc.append("postgres")
    		url = urlparse.urlparse(os.environ["DATABASE_URL"])
    		db = psycopg2.connect(database=url.path[1:],user=url.username,password=url.password,host=url.hostname,port=url.port)
                cur = db.cursor()
...
##                                print conf
##                                if conf > 0.9:
##                                        out_msg = 'Enroll to online banking at https://webbanking.comerica.com/Comerica/Enrollment/Enroll.aspx'
                                words = word_tokenize(txt_msg)
                                tokens = nltk.word_tokenize(txt_msg)
                                tagged = nltk.pos_tag(tokens)
                                entities = nltk.ne_chunk(tagged)
                                        ...
                                        if(ps.stem(w).lower()== 'atm'):
                                                for subtree in entities.subtrees():
                                                        if subtree.label() == 'GPE':
                                                                ...
                                                                print "if"
                                                                location = subtree.leaves()[0][0]
                                                                if(location.lower()=="livonia"):
                                                                        flag='Y'
                                                                        json_data_final={"recipient": {"id": sender_id}, "message":{"attachment": {"type": "template", "payload": {"template_type":"button","text":"List Of ATM's in Livonia","buttons":[{"type":"web_url","url":"http://maps.google.com/?q=42.397274,-83.406500","title":"Five Mile Rd"},{"type":"web_url","url":"http://maps.google.com/?q=42.411814,-83.430477","title":"W 6 Mile Rd"},{"type":"web_url","url":"http://maps.google.com/?q=42.417011,-83.418955","title":"N Laurel Park Dr"}]}}}}
                                                                else:
                                                                        out_msg="We don't have any ATM'S in this loaction."   
                                                        else:
                                                                print "else"
                ...
                print "sender id %s" %sender_id
                json_data_typing_off={"recipient": {"id": sender_id}, "sender_action": "typing_off"}
                postingMessageComerica(json_data_typing_off);
                if(flag=='N'):
                        json_data_final={"recipient": {"id": sender_id}, "message": {"text": out_msg}}
                                      
                postingMessageComerica(json_data_final);
                mid=''
                postbak_msg = dataDict["entry"][0]["messaging"][0]
                if ('message' not in postbak_msg):
                        mid=''
                                        ...
                                        if cur.fetchone():
                                                if txt_msg.lower() == 'hola':
                                                        ...
                                                        if "Your quote id" in speech:
                                                                ...
                                                                flag='Y'
                                                                json_data_final={"recipient":{"id":sender_id},"message": {"attachment": {"type": "file","payload": {"url":"https://chatbot-final.herokuapp.com/resources/pdf/Quote_Details.pdf" }}}}
                                                        out_msg= out_resp["result"]["fulfillment"]["speech"]
                                        else:
                                                flag='Y'
                                                json_data_final={"recipient":{"id":sender_id},"message": {"attachment": {"type": "template","payload": {"template_type": "generic","elements": [{"title": "If you are an existing agent, please logon for MGA bot.","image_url": "","buttons": [{"type": "account_link","url": "https://chatbot-final.herokuapp.com/login"}]}]}}}}
...
def postingMessageComerica(json_data):
        req = urllib2.Request('https://graph.facebook.com/v2.6/me/messages?access_token=EAAB1ZAGm7z1IBAJ8mOOZBZAE8f4v5jDQAT97GLjW6gQ3j11AFsoop4174s02x0cwKiKgSh567QlmUoPK8nguEIObk2KBmdXlTalDD5m6YjWVkqWwD1ZCck7vuYuaXdQVoDs3EpiNZBUUSavvXRzZAaYaUaCgYKPxNKrrsLS4XUqYO2dD4tFgmXNRj4irkyAWEZD')
        req.add_header('Content-Type', 'application/json')
        response = urllib2.urlopen(req, json.dumps(json_data))
        return response;
...
def incoming_sms():
        ...
        if cur.fetchone():                              
                ...
                if action =='list':
                        ...
                        cur.execute("select ab.available_bal from chat_user u,account_balance ab where ab.olb_id=u.olb_id and ab.acct_type='Checking' and u.mobile_no=%s"%from_number)
                        balance=cur.fetchone()[0]
                        out_msg = 'Checking account balance is $%s'%balance
                        ...
                        if not retailname:
                                ...
                                for desc in retailname: 
                                        querystring="'%"+desc.lower()+"%'"
                                        cur.execute('select sum(tran_amount::numeric) from transaction_details where olb_id = %s and tran_desc like %s'%(olb_id,querystring))
                                        balance=cur.fetchone()[0]
                                        if balance is None:
                                                balance = 0
                                        else:
...
def chatapiai():
	...
	from_number="'"+from_number+"'"
        print from_number
        cur.execute('select mobile_no from chat_user where auth_code=%s'%from_number)
	...
	print from_number
        ...
        if cur.fetchone():                              
                ...
                if language_val1 == 'es' or language_val2== 'es':                        
                        workspace_id = 'b9757313-7979-48d7-88ac-e33241397980'
		elif language_val1 == 'ja' or language_val1 == 'zh-TW':
...
def processRequest(req):
	...
	cur = db.cursor()
		...
		quote_id_db="'"+quote_id+"'"
		name_db="'"+name+"'"
		state_db="'"+state+"'"
		zipcode_db="'"+zipcode+"'"
		policyterm_db="'"+policyterm+"'"
		builtyear_db="'"+builtyear+"'"
		type1_db="'"+type1+"'"
		area_db="'"+area+"'"
		date_created="'"+time.strftime('%Y-%m-%d')+"'"
				...
				print "inyear"							
				cur.execute("insert into quote_details values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"%(quote_id_db,name_db,state_db,zipcode_db,policyterm_db,builtyear_db,type1_db,area_db,date_created))
		...
		status_db="'"+status+"'"
                claim_type='property'
		claim_type_db="'"+claim_type+"'"
                loss_descr='description'
		loss_descr_db="'"+loss_descr+"'"
                claim_id=id_generator()
		claim_id_db="'"+claim_id+"'"
		losstype_db="'"+losstype+"'"
		lossdate_db="'"+lossdate+"'"
		lossamount_db="'"+lossamount+"'"		
		cur.execute("insert into claim_details values(%s,%s,%s,%s,%s,%s,%s)"%(claim_id_db,claim_type_db,losstype_db,lossdate_db,loss_descr_db,lossamount_db,status_db))
                ...
                if flag.lower() == 'y' or flag.lower() == 'yes':
                        ...
                        if cur.fetchone():
                                ...
                                cur.execute('select mobile_no from chat_user where auth_code=%s'%sender_id)
                                mobile_num=cur.fetchone()[0]
                                print 'mobile2'                        
                        print mobile_num                                               
                        if(mobile_num=='+19193997682'):
